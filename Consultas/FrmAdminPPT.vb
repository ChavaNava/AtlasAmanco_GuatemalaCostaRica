...
Public Class FrmAdminPPT
    ...
    Private Sub ToolStripButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbCerrar.Click
        Close()
    End Sub
    ...
    Private Sub LimpiaObjetos()
        ...
        txtSobrepeso.Text = "0" & GDECIMAL & "00"
        txtPorcsobrepeso.Text = "0" & GDECIMAL & "00"
    ...
    Private Sub ReporteEtiquetaToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If DGPPT.CurrentRowIndex > -1 Or DGPPT.CurrentCell.RowNumber > 0 Then
            xORIGINAL = "  ============ COPIA   "
            If DGPPT(DGPPT.CurrentCell.RowNumber, 5) = "PRODUCCION" Or DGPPT(DGPPT.CurrentCell.RowNumber, 5) = "EN PROCESO" Or DGPPT(DGPPT.CurrentCell.RowNumber, 5) = "SOBREPESO" Then
                imprimir_PT()
            Else
                imprimir_SC()
            End If
    ...
    Private Sub tsbImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbImprimir.Click
        If DGPPT.CurrentRowIndex > -1 Or DGPPT.CurrentCell.RowNumber > 0 Then
            xORIGINAL = "  ============ COPIA   "
            If DGPPT(DGPPT.CurrentCell.RowNumber, 6) = "PRODUCCION" Or DGPPT(DGPPT.CurrentCell.RowNumber, 6) = "EN PROCESO" Or DGPPT(DGPPT.CurrentCell.RowNumber, 6) = "SOBREPESO" Then
                imprimir_PT()
            Else
                imprimir_SC()
            End If
    ...
    Sub imprimir_PT()
        ...
        PFolioAtlas = DGPPT(DGPPT.CurrentCell.RowNumber, 28)
        ...
        QRY = QRY & " PPT.Tara, PPT.Peso_Neto, PPT.Sobrepeso, PPT.Peso_teorico, PPT.PuestoTrabajo, PPT.Usuario, PPT.Tipo_PT, FolioR "
        ...
        If PTipoPT.Trim = "R" Then
            PrintLine(1, "  * * * (" & PTipoPT.Trim & ": " & PFolioR & " ) PRODUCCION FINAL  ")
        End If

        If PTipoPT.Trim = "S" Then
            PrintLine(1, "  * * * (" & PTipoPT.Trim & ") SOBREPESO  ")
        End If
    ...
    Sub imprimir_SC()
        ...
        PrintLine(1, "TURNO: " & PTurno.Trim & "   F.TURNO: " & PFechaTurno.Trim)
        PrintLine(1)
        If PTipoScrap = "T" Then
            PrintLine(1, "  * * * (" & PTipoScrap & ") SCRAP FINAL  ")
        End If
        If PTipoScrap = "R" Then
            PrintLine(1, "  * * * (" & PTipoScrap & ": " & PFolioR & " ) SCRAP FINAL  ")
        End If
        If PTipoScrap = "G" Then
            PrintLine(1, "  * * * (" & PTipoScrap & ") PURGA  ")
        End If
        If PTipoScrap = "P" Then
            PrintLine(1, "  * * * (" & PTipoScrap & ") SPRUT  ")
        End If
        PrintLine(1)
        PrintLine(1, " TIPO PROD.: " & PGrpprod.Trim & "    " & PGrpproddesc.Trim)
        PrintLine(1)
    ...
    Private Sub chkResumenSeccion_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'If chkResumenSeccion.Checked = True Then
        '    grpResumenSeccion.Height = 150
        '    grpResumenSeccion.Top = grpTotales.Location.Y - 150
        '    grpResumenOrden.Height = grpResumenSeccion.Top - 55

        'Else
        '    grpResumenSeccion.Height = 44
        '    grpResumenSeccion.Top = grpTotales.Location.Y - 45
        '    grpResumenOrden.Height = grpResumenSeccion.Top - 55

        'End If

        'scrDetalles.Height = grpResumenOrden.Height - 20
        'scrDetalles.Minimum = 0
        'scrDetalles.Maximum = grpResumenOrden.Height - 60
        'DGPPT.Height = scrDetalles.Value
    End Sub
    Private Sub DGPPT_MouseDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles DGPPT.MouseDoubleClick
        ...
        Try
            If DGPPT.CurrentRowIndex > -1 Then
                ...
                TxtFolioSel.Text = DGPPT.Item(DGPPT.CurrentCell.RowNumber, 28).ToString.Trim
                PTipoPT = DGPPT.Item(DGPPT.CurrentCell.RowNumber, 6).ToString.Trim

                Calcula_Cantidades()
                llena_novedades()

                If GFOTOS = "S" Then
                    If PTipoPT.Trim = "PRODUCCION" Then
                        TxtCantidadSel = DGPPT.Item(DGPPT.CurrentCell.RowNumber, 7).ToString.Trim
                    End If
                    If PTipoPT.Trim = "SOBREPESO" Or PTipoPT.Trim = "EN PROCESO" Then
                        TxtCantidadSel = DGPPT.Item(DGPPT.CurrentCell.RowNumber, 9).ToString.Trim
                    End If
                    If PTipoPT.Trim = "SCRAP" Or PTipoPT.Trim = "SPRUT" Or PTipoPT.Trim = "PURGA" Then
                        TxtCantidadSel = DGPPT.Item(DGPPT.CurrentCell.RowNumber, 11).ToString.Trim
                    End If

                    '         GIMAGENNOM = GIMAGENURI & SessionUser._sCentro.Trim & "-PT-O" & TxtOrdProdSel.Text & "F" & TxtFolioSel.Text & ".jpg"
                    '           GIMAGENDATOS = SessionUser._sCentro.Trim & "-" & TxtOrdProdSel.Text & "-" & TxtFolioSel.Text & "-" & TxtCantidadSel
                    '            Try
    ...
    Private Sub Calcula_Cantidades()
        ...
        Qfup = Qfup & "  FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos<>0 AND tipo_PT IN ('T','R') AND notifica in ('0','1','4')) "
        ...
        Qfup = Qfup & " FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos<>0 AND tipo_PT IN ('P','S') AND notifica in ('2','3','4'))) PTYS "
    ...
    Private Sub ExportarConsultaToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If (LecturaBD.HasRows) Then
            ...
            FileOpen(1, GEXPORTACIONES.Trim & "\ExpExtrusion.txt", OpenMode.Output)
            ...
            Dim strFileName As String = GEXPORTACIONES.Trim & "\ExpExtrusion"
        ...
        '        'System.IO.File.Replace(strFileName, strFileName1, strFileName2)
    ...
    Private Sub tsbConsultar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbConsultar.Click
        ...
        Dim xSOBREPESO As Decimal
        Dim xPORCSOBREPESO As Decimal
        ...
        Qd = Qd & " PTE.Codigo CODIGO, PTE.Descr MATERIAL, PTYS.Tipo TIPO, PTYS.tipoprod PROCESO, PTYS.Tramos U_PRODUCCION, PTYS.PRODUCCION K_PRODUCCION, PTYS.sTramos U_SEPARADAS, PTYS.SEPARADA K_SEPARADA, PTYS.xTramos U_RECHAZO, PTYS.SCRAP K_SCRAP, PTYS.SPRUT K_SPRUT, PTYS.PURGA K_PURGA, "
        Qd = Qd & " CONVERT(DECIMAL(18,2),ROUND(((CASE PTYS.SCRAP WHEN 0 THEN 0 ELSE (PTYS.SCRAP/PTYS.SCRAP) END)*100),2))Porc_SCRAP, "
        Qd = Qd & " (PTYS.PRODUCCION + PTYS.SCRAP) TOTAL, (PTYS.Tramos * PTYS.PTEORICO) PESO_TEORICO, "
        Qd = Qd & " (PTYS.PRODUCCION - (PTYS.Tramos * PTYS.PTEORICO)) SOBREPESO, "
        Qd = Qd & " CONVERT(DECIMAL(18,2),ROUND(((CASE PTYS.PRODUCCION WHEN 0 THEN 0 ELSE ((PTYS.PRODUCCION/(PTYS.TRAMOS * PTYS.PTEORICO))-1) END)*100),2)) Porc_SOBREPESO, "
        ...
        Qd = Qd & " FROM (((sELECT Orden_Produccion,  Fecha_Pesaje, Hora_Pesaje, Turno,  Tipo_PT as Tipo, "
        ...
        Qd = Qd & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, Compuesto1, Porcentaje1, Compuesto2, Porcentaje2, LTCompuestos, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('T','R') and notifica in ('0','1','4')) "

        Qd = Qd & " UNION ALL (SELECT Orden_Produccion,  Fecha_Pesaje, Hora_Pesaje, Turno,  Tipo_PT as Tipo, "
        ...
        Qd = Qd & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, Compuesto1, Porcentaje1, Compuesto2, Porcentaje2, LTCompuestos, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('P') and notifica = '3' ) "
        ...
        Qd = Qd & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, Compuesto1, Porcentaje1, Compuesto2, Porcentaje2, LTCompuestos, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('S') and notifica = '2' ) "

        Qd = Qd & " UNION ALL (SELECT Orden_Produccion, Fecha_Pesaje, Hora_Pesaje, Turno, Tipo_Scrap as Tipo, "
        ...
        Qd = Qd & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, Compuesto1, Porcentaje1, Compuesto2, Porcentaje2, LTCompuestos, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE Tipo_Scrap in ('T','R') AND notifica in ('0','1','4')) "
        Qd = Qd & " UNION ALL (SELECT Orden_Produccion, Fecha_Pesaje, Hora_Pesaje, Turno, Tipo_Scrap as Tipo, "
        ...
        Qd = Qd & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, Compuesto1, Porcentaje1, Compuesto2, Porcentaje2, LTCompuestos, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE tramos = 0 AND Tipo_Scrap in ('P') AND notifica in ('0','1','4')) "
        Qd = Qd & " UNION ALL (SELECT Orden_Produccion, Fecha_Pesaje, Hora_Pesaje, Turno, Tipo_Scrap as Tipo,  "
        ...
        Qd = Qd & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, Compuesto1, Porcentaje1, Compuesto2, Porcentaje2, LTCompuestos, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE Tipo_Scrap in ('G') AND notifica = '5')) PTYS "
        Qd = Qd & " INNER JOIN [" & SessionUser._sCentro.Trim & "_OrdenProduccion] OP ON PTYS.Orden_Produccion = OP.Orden_Produccion) "
        ...
        Qt = Qt & " PTE.Codigo Codigo, PTE.Descr Material, SUM(PTYS.TRAMOS) U_Produccion, SUM(PTYS.PRODUCCION) K_Produccion,  SUM(PTYS.sTramos) U_Separada, SUM(PTYS.SEPARADA) K_Separada,"
        Qt = Qt & " SUM(PTYS.xTramos) U_Rechazo, SUM(PTYS.SCRAP) K_Scrap, SUM(PTYS.SPRUT) K_Sprut, SUM(PTYS.PURGA) K_Purga, SUM(PTYS.TRAMOS * PTYS.PTEORICO) Peso_Teorico , (SUM(PTYS.PRODUCCION) - SUM(PTYS.TRAMOS * PTYS.PTEORICO)) Sobrepeso,  "
        Qt = Qt & " CONVERT(DECIMAL(18,2),ROUND(((CASE SUM(PTYS.PRODUCCION) WHEN 0 THEN 1 ELSE ((SUM(PTYS.PRODUCCION)/SUM(PTYS.TRAMOS * PTYS.PTEORICO))-1) END)*100),2))  Porc_Sobrepeso, "
        ...
        Qt = Qt & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('T','R') and notifica in ('0','1','4')) "
        ...
        Qt = Qt & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('P') and notifica = '3' ) "
        Qt = Qt & " UNION ALL (SELECT Orden_Produccion,  Fecha_Pesaje, Hora_Pesaje,  Turno, "
        ...
        Qt = Qt & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('S') and notifica = '2' ) "
        Qt = Qt & " UNION ALL (SELECT Orden_Produccion, Fecha_Pesaje, Hora_Pesaje,  Turno, "
        ...
        Qt = Qt & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE Tipo_Scrap in ('T','R') AND notifica in ('0','1','4')) "
        Qt = Qt & " UNION ALL (SELECT Orden_Produccion, Fecha_Pesaje, Hora_Pesaje,  Turno, "
        ...
        Qt = Qt & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE tramos = 0 AND Tipo_Scrap in ('P') AND notifica in ('0','1','4')) "
        Qt = Qt & " UNION ALL (SELECT Orden_Produccion, Fecha_Pesaje, Hora_Pesaje,  Turno, "
        ...
        Qt = Qt & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE Tipo_Scrap in ('G') AND notifica = '5')) PTYS "
        Qt = Qt & " INNER JOIN [" & SessionUser._sCentro.Trim & "_OrdenProduccion] OP ON PTYS.Orden_Produccion = OP.Orden_Produccion) "
        ...
        Qs = Qs & " (SUM(PTYS.PRODUCCION) - SUM(PTYS.TRAMOS * PTYS.PTEORICO)) Sobrepeso,  "
        Qs = Qs & " CONVERT(DECIMAL(18,2),ROUND(((CASE SUM(PTYS.PRODUCCION) WHEN 0 THEN 1 ELSE ((SUM(PTYS.PRODUCCION)/SUM(PTYS.TRAMOS * PTYS.PTEORICO))-1) END)*100),2))  Porc_Sobrepeso, "
        ...
        Qs = Qs & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('T','R') and notifica in ('0','1','4')) "
        ...
        Qs = Qs & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('P') and notifica = '3' ) "
        ...
        Qs = Qs & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('S') and notifica = '2' ) "
        ...
        Qs = Qs & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE Tipo_Scrap in ('T','R') AND notifica in ('0','1','4')) "
        Qs = Qs & " UNION ALL (SELECT Orden_Produccion, Fecha_Pesaje, Hora_Pesaje,  Turno, "
        ...
        Qs = Qs & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE tramos = 0 AND Tipo_Scrap in ('P') AND notifica in ('0','1','4')) "
        Qs = Qs & " UNION ALL (SELECT Orden_Produccion, Fecha_Pesaje, Hora_Pesaje,  Turno, "
        ...
        Qs = Qs & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE Tipo_Scrap in ('G') AND notifica = '5')) PTYS "
        Qs = Qs & " INNER JOIN [" & SessionUser._sCentro.Trim & "_OrdenProduccion] OP ON PTYS.Orden_Produccion = OP.Orden_Produccion) "
        ...
        Qtp = Qtp & " (SUM(PTYS.PRODUCCION) - SUM(PTYS.TRAMOS * PTYS.PTEORICO)) SOBREPESO, "
        Qtp = Qtp & " CONVERT(DECIMAL(18,2),ROUND(((CASE SUM(PTYS.PRODUCCION) WHEN 0 THEN 1 ELSE ((SUM(PTYS.PRODUCCION) / (SUM(PTYS.TRAMOS * PTYS.PTEORICO)))-1) END)*100),2)) Porc_SOBREPESO, "
        ...
        Qtp = Qtp & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('T','R') and notifica in ('0','1','4')) "
        ...
        Qtp = Qtp & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('P') and notifica = '3' ) "
        Qtp = Qtp & " UNION ALL (sELECT Orden_Produccion,  Fecha_Pesaje, Hora_Pesaje,  Turno, "
        ...
        Qtp = Qtp & " Consecutivo_SAP, Planta, Bascula, Usuario, PackID, Fecha_turno, PuestoTrabajo, peso_teorico as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoProductoTerminado] WHERE tramos <> 0 AND Tipo_PT in ('S') and notifica = '2' ) "
        ...
        Qtp = Qtp & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE Tipo_Scrap in ('T','R') AND notifica in ('0','1','4')) "
        Qtp = Qtp & " UNION ALL (SELECT Orden_Produccion, Fecha_Pesaje, Hora_Pesaje,  Turno, "
        ...
        Qtp = Qtp & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE tramos = 0 AND Tipo_Scrap in ('P') AND notifica in ('0','1','4')) "
        Qtp = Qtp & " UNION ALL (SELECT Orden_Produccion, Fecha_Pesaje, Hora_Pesaje,  Turno, "
        ...
        Qtp = Qtp & " Consecutivo_SAP, Planta, Bascula, Usuario, '0', Fecha_turno, PuestoTrabajo, 0 as PTEORICO FROM [" & SessionUser._sCentro.Trim & "_PesoScrap] WHERE Tipo_Scrap in ('G') AND notifica = '5')) PTYS "
        Qtp = Qtp & " INNER JOIN [" & SessionUser._sCentro.Trim & "_OrdenProduccion] OP ON PTYS.Orden_Produccion = OP.Orden_Produccion) "
        Qtp = Qtp & " INNER JOIN MCPC_EquipoBasico EB ON PTYS.planta = EB.Planta AND  PTYS.PuestoTrabajo = EB.Equipo_basico  AND EB.Tpo_equipo_basico='" & EXTINY & "' "
        ...
        Qfup = Qfup & " FROM (([" & SessionUser._sCentro.Trim & "_HorasParo] THP INNER JOIN [" & SessionUser._sCentro.Trim & "_OrdenProduccion] OP ON THP.Centro = OP.planta AND THP.ordenProd = OP.Orden_Produccion )"
        Qfup = Qfup & " INNER JOIN MCPC_EquipoBasico EB ON THP.centro = EB.Planta AND THP.PuestoTrabajo = EB.Equipo_basico AND EB.Tpo_equipo_basico='" & EXTINY & "' "
        Qfup = Qfup & " INNER JOIN ProductoTerminadoExtrusion PTE ON PTE.CENTRO= THP.centro and PTE.Codigo = OP.Producto )"
        Qfup = Qfup & " INNER JOIN CatCausas CC  ON CC.Centro=THP.Centro AND CC.grpprod=PTE.grpprod AND CC.CodCausa=THP.CodCausa   "
        Qfup = Qfup & " INNER JOIN CatAgrupaciones CA ON CC.centro= CA.centro AND CC.codcausagrupa = CA.codcausagrupa  AND CA.status = 'A' "
        Qfup = Qfup & " where THP.CodCausa='0000' and THP.status<>'9')"
        Qfup = Qfup & " union all (SELECT THP.Centro, THP.ordenProd, THP.fecha, THP.turno, THP.PuestoTrabajo, THP.CodCausa, 0 as HREPORTADAS, horasparo as POR_PROGRAMA, 0 as MANTENIMIENTO, 0 as POR_SETUP, 0 as OTROS"
        ...
        If CB2.Checked = True Then
            ...
            If TxtOrdProd.Text.Trim = "*" Then
                ...
                Qt = Qt & " GROUP BY PTYS.PuestoTrabajo, PTE.Codigo, PTE.Descr "
                Try
                ...
                Try
                    ...
                    If myDataReader.Read() Then
                        ...
                        xSOBREPESO = "" & myDataReader(4).ToString
                        xPORCSOBREPESO = "" & myDataReader(5).ToString
                        xTOTUNSCRAP = "0" & myDataReader(6).ToString
                        xTOTKILOSSCRAP = "0" & myDataReader(7).ToString
                        xPORCSCRAP = "" & myDataReader(8).ToString
                        xTOTKILOSSPRUT = "0" & myDataReader(9).ToString
                        xPORCSPRUT = "" & myDataReader(10).ToString
                        xTOTKILOSPURGA = "0" & myDataReader(11).ToString
                        ...
                        txtSobrepeso.Text = Format(xSOBREPESO, xFormato)
                        txtPorcsobrepeso.Text = Format(xPORCSOBREPESO, xFormato)
                ...
                Qfup = Qfup & "   GROUP by HPTOTAL.centro "
                Try
                    objCmd = New SqlCommand
                    objCmd.CommandType = CommandType.Text
                    objCmd.Connection = objCnnAmanco
                    ...
                    myDataReader = objCmd.ExecuteReader()
                    If myDataReader.Read() Then
                        xTotHorastrab = "0" & LecturaBD(0)
                        xTotHorasprog = "0" & LecturaBD(1)
                        xTotHorasmant = "0" & LecturaBD(2)
                        xTotHorassetu = "0" & LecturaBD(3)
                        xTotHorasotro = "0" & LecturaBD(4)
            ...
            If TxtOrdProd.Text <> "*" Then
                ...
                Catch ex As Exception
                    MessageBox.Show("Error Conexion :" & ex.Message, "DATOS RESUMEN PESAJES")
                End Try
            ...
            If TxtOrdProd.Text = "*" Then
                ...
                Try
                    ...
                    If myDataReader.Read() Then
                        ...
                        xTOTKILOSPROD = "0" & myDataReader(1).ToString
                        xTOTUNPROCESO = "0" & myDataReader(2).ToString
                        xTOTKILOSPROCESO = "" & myDataReader(3).ToString
                        xSOBREPESO = "" & myDataReader(4).ToString
                        xPORCSOBREPESO = "" & myDataReader(5).ToString
                        xTOTUNSCRAP = "0" & myDataReader(6).ToString
                        xTOTKILOSSCRAP = "0" & myDataReader(7).ToString
    ...
    Private Sub ToolStripButton1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbExportar.Click
        Dim strTempQuery As String = ""
        Dim xLinea As String = ""
        Dim i As Integer = 0
        Dim FExport As String
