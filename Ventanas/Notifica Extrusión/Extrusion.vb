...
#Region "Variables miembro"
    ...
    'Variables Consulta Materiales Consumo
    Dim DGVM_Mov As String
    Dim DGVM_Cod As String
    Dim DGVM_Des As String
    Dim DGVM_Alm As String
    Dim DGVM_UM As String
    Dim DGVM_Por As Decimal
    Dim DGVM_Can As Decimal
    Dim DGVM_PU As Decimal
    Dim DGVM_Cantidad As Decimal
    Dim DGVM_Peso As Decimal
    Dim DGVM_Litros As Decimal
    Dim DGVM_Tip As String
    ...
    Private Sub TPesoNeto_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TPesoNeto.TextChanged
        ...
        If TOrden.Text.Trim <> "" Then
            If TCodPT.Text.Trim <> "" Then
                If TtramosNoti.Text.Trim <> "0" Then
                    ConsumosCompuestos(DGVResumen, TOrden.Text.Trim, TCodPT.Text.Trim, TtramosNoti.Text, TPesoNeto.Text.Trim, ConfigCompuestos._Lista.Trim)
                    MaterialesConsumo(DGVMateriales, TOrden.Text.Trim, TCodPT.Text.Trim, TtramosNoti.Text, ConfigCompuestos._Lista.Trim)
                End If
            End If
        End If
...
#Region "METODOS"
    ...
    Private Sub ConsumosCompuestos(ByVal DataGV As DataGridView, IdOrden As String, IdProducto As String, IdPiezas As Integer, IdPeso As String, IdLista As String)
        Dim NDSCompuesto As New DataSet
        Dim DGV_Mov As String
        Dim DGV_Cod As String
        Dim DGV_Des As String
        Dim DGV_Alm As String
        Dim DGV_UM As String
        Dim DGV_Por As Decimal
        Dim DGV_Can As Decimal
        Dim DGV_Tip As String

        DGVResumen.DataSource = Nothing
        DGVResumen.Rows.Clear()

        Q = ""
        Q = "PA_ConsumoCompuestoSAP '" & SessionUser._sCentro.Trim & "', '" & IdOrden.Trim & "', '" & IdProducto.Trim & "', '" & IdPiezas & "', '" & IdPeso.Trim & "', '" & IdLista.Trim & "',1 "
        LecturaQry(Q)
        ...
        Do While (LecturaBD.Read)
            ...
            Dim row As String() = New String() {DGV_Cod, DGV_Des, DGV_Por, Format(DGV_Can, "#,##0.00")}
            DataGV.Rows.Add(row)
            DataGV.Refresh()
        Loop
    ...
    Private Sub MaterialesConsumo(ByVal DataGV As DataGridView, IdOrden As String, IdProducto As String, IdPiezas As Integer, IdLista As String)


        If IdPiezas <> DGVM_Can Then
            DGVMateriales.DataSource = Nothing
            DGVMateriales.Rows.Clear()

            Q = ""
            Q = "PA_ConsumoCompuestoSAP '" & SessionUser._sCentro.Trim & "', '" & IdOrden.Trim & "', '" & IdProducto.Trim & "', '" & IdPiezas & "', '', '" & IdLista.Trim & "',2"
            LecturaQry(Q)

            Do While (LecturaBD.Read)
                DGVM_Mov = LecturaBD(0)
                DGVM_Cod = LecturaBD(1)
                DGVM_Des = LecturaBD(2)
                DGVM_Alm = LecturaBD(3)
                DGVM_UM = LecturaBD(4)
                DGVM_Por = LecturaBD(5)
                DGVM_Can = LecturaBD(6)
                DGVM_PU = LecturaBD(7)
                DGVM_Tip = LecturaBD(8)

                Select Case DGVM_UM
                    Case Is = "KG"
                        DGVM_Cantidad = DGVM_Can
                        DGVM_Peso = (DGVM_Can * DGVM_PU)
                        DGVM_Litros = 0.0
                    Case Is = "L"
                        DGVM_Cantidad = 0.0
                        DGVM_Peso = 0.0
                        DGVM_Litros = (DGVM_Can * DGVM_PU)
                End Select

                Dim row As String() = New String() {DGVM_Cod, DGVM_Des, Format(DGVM_Cantidad, "#,##0"), Format(DGVM_Peso, "#,##0.000"), Format(DGVM_Litros, "#,##0.000"), DGVM_UM}
                DataGV.Rows.Add(row)
                DataGV.Refresh()
...
#End Region
    ...
    Private Sub BtnAddC_Click(sender As System.Object, e As System.EventArgs) Handles BtnAddC.Click
        FrmAlta_Comp.Text = "Alta Nuevo Compuesto"
        FrmAlta_Comp.ShowDialog()
    End Sub

    Private Sub DGVResumen_CurrentCellChanged(sender As System.Object, e As System.EventArgs) Handles DGVResumen.CurrentCellChanged
        Dim oldRowIndex As Object
        oldRowIndex = DGV.CurrentCellAddress.Y
        If oldRowIndex <> -1 Then
            Try
                ListaCompuestos.cCodigo = DGV.Rows(DGV.CurrentCell.RowIndex).Cells(0).Value.ToString
                ListaCompuestos.cDesc = DGV.Rows(DGV.CurrentCell.RowIndex).Cells(1).Value.ToString
                ListaCompuestos.cPorc = DGV.Rows(DGV.CurrentCell.RowIndex).Cells(2).Value.ToString
                ListaCompuestos.cCantidad = DGV.Rows(DGV.CurrentCell.RowIndex).Cells(3).Value.ToString
                ListaCompuestos.cCodRemp = DGV.Rows(DGV.CurrentCell.RowIndex).Cells(4).Value.ToString
                ListaCompuestos.cDescRemp = DGV.Rows(DGV.CurrentCell.RowIndex).Cells(5).Value.ToString
            Catch ex As Exception
    ...
    Private Sub DGVResumen_CellValueChanged(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DGVResumen.CellValueChanged
        Dim TotalPorc As Decimal
        Dim cCodigo As String
        Dim cPorcentaje As Decimal
        Dim cCantidad As Decimal
        Dim TPorc As Decimal

        TotalPorc = Funciones.Sumar("Porcent", DGVResumen)
        For j = 0 To DGVResumen.RowCount - 1
            cCodigo = DGVResumen.Rows(j).Cells(0).Value.ToString()
            cPorcentaje = DGVResumen.Rows(j).Cells(2).Value.ToString()
            cCantidad = DGVResumen.Rows(j).Cells(3).Value.ToString()

            If j = DGVResumen.RowCount - 1 Then
                DGVResumen.Rows(j).Cells(2).Value = (100 - TPorc)
            Else
                TPorc = TPorc + cPorcentaje
            End If
